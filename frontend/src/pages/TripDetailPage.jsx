...
function InfoRow({ icon: Icon, label, value, capitalize }) {
  return (
    <div className="flex items-start gap-3">
      <div className="mt-0.5 text-[#8a9e7c] shrink-0">
        {Icon && <Icon />}
      </div>
      <div className="min-w-0">
        <span className="text-xs text-[#8a9e7c]">{label}</span>
        <p className={`text-sm font-medium text-[#2d4a24] ${capitalize ? 'capitalize' : ''} truncate`}>
          {value}
        </p>
      </div>
    </div>
...
function SectionHeading({ children }) {
  return (
    <h3 className="text-xs font-semibold text-[#8a9e7c] uppercase tracking-wider mb-3">
      {children}
    </h3>
  );
}
...
function FeasibilityBadge({ status }) {
  const styles = {
    feasible: 'bg-[#eef4e8] text-[#4a6741]',
  ...
  const cls = styles[status] || 'bg-[#f0f0ee] text-[#8a9e7c]';
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-[#8a9e7c]">Feasibility</span>
...
function Skeleton({ className = '' }) {
  return <div className={`animate-pulse bg-[#e0ddd6] rounded ${className}`} />;
}
  ...
  return (
    <div className="w-full max-w-full mt-8 px-8">
      <div className="flex gap-6">
        <div className="w-[480px] shrink-0 bg-white border border-[#e8e4da] rounded p-6 space-y-4">
  ...
  if (editing) {
    return (
      <div className="w-full max-w-4xl mx-auto mt-8 p-6">
        <div
          className="bg-white rounded-lg shadow-sm border border-[#e8e4da] overflow-hidden"
          style={{ borderLeft: '4px solid #4a6741' }}
        >
          <div className="p-8">
            <h1 className="text-xl font-bold text-[#2d4a24] mb-6">Edit Trip</h1>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-[#5a6b4e] mb-1.5">Name</label>
                    <input
                      name="name"
                      value={editForm.name}
                      onChange={handleEditChange}
                      className="w-full border border-[#d4cfbf] rounded-lg px-3 py-2 text-sm text-[#2d4a24] focus:outline-none focus:ring-2 focus:ring-[#4a6741] focus:border-transparent"
                    />
                  </div>
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-[#5a6b4e] mb-1.5">Travel Date</label>
                      <input
                        type="date"
                        name="travel_date"
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-[#5a6b4e] mb-1.5">Status</label>
                      <select
                        name="status"
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-[#5a6b4e] mb-1.5">Number of Days</label>
                      <input
                        type="number"
                        name="number_of_days"
                        min="1"
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-[#5a6b4e] mb-1.5">
                      ...
                      />
                      <div className="flex justify-between text-xs text-[#8a9e7c] mt-0.5">
                        <span>4h</span>
                        <span>16h</span>
                      </div>
                    </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-[#5a6b4e] mb-2">Travel Style</label>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                            editForm.travel_style === s
                              ...
                              : 'bg-white text-[#5a6b4e] border-[#d4cfbf] hover:border-[#4a6741] hover:text-[#4a6741]'
                          }`}
                        >
                          {STYLE_LABELS[s]}
                        </button>
            ...
            <div className="mt-8 pt-5 border-t border-[#e0ddd6] flex items-center gap-3">
              <button
                onClick={handleSave}
                disabled={saving}
                className="bg-[#4a6741] text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-[#3d5a35] disabled:opacity-50 transition-colors"
              >
                {saving ? 'Saving...' : 'Save Changes'}
              </button>
              <button
                onClick={handleCancel}
                className="text-[#5a6b4e] px-5 py-2.5 rounded-lg text-sm font-medium border border-[#d4cfbf] hover:bg-[#eef4e8] transition-colors"
              >
                Cancel
              </button>
            </div>
    ...
    <div className="w-full max-w-full mt-8 px-8" style={{ height: 'calc(100vh - 76px)' }}>
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between mb-4 shrink-0">
          ...
          {isOwner() && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-sm font-medium text-[#4a6741] border border-[#d4cfbf] rounded-lg px-3.5 py-2 hover:bg-[#eef4e8] transition-colors"
            >
              <PencilIcon />
              Edit
            </button>
        ...
        <div className="flex gap-6 flex-1 min-h-0">
          {/* Left: Info */}
          <div className="w-[480px] shrink-0 bg-white border border-[#e8e4da] rounded-lg p-6 overflow-y-auto">
          <div className="flex items-start justify-between mb-6">
            <h1 className="text-xl font-bold text-[#2d4a24] leading-tight">{trip.name}</h1>
            <span
        ...
        {trip.route_polyline && (
          <div className="flex-1 min-w-0 h-full flex flex-col">
            <div className="flex-1 bg-white border border-[#e8e4da] rounded-lg overflow-hidden">
              <RouteMap
                origin={
