        ...
        <div className="flex-1 bg-card px-6 md:px-8 py-6 md:py-8 flex flex-col justify-center w-full lg:max-w-[480px]">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-brand disabled:bg-muted text-brand-light border border-brand-hover rounded-lg text-base font-semibold disabled:cursor-not-allowed cursor-pointer active:scale-[0.98] transition-transform"
            >
              {loading ? (
            ...
            <span className="text-sm text-muted whitespace-nowrap">or continue with</span>
            <div className="flex-1 h-px bg-line" />
          </div>
            ...
            {['Google', 'Facebook', 'Apple'].map((provider) => (
              <button
                ...
                className="flex-1 h-11 border border-line rounded-lg bg-input text-sm font-medium text-muted cursor-not-allowed opacity-60"
              >
                {provider}
              </button>
            ))}
